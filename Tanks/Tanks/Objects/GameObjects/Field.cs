        ...
        public void AddObject (AddableObjects obj, InGameEngine engine, Coordinate position = null)
        {
            switch (obj)
            {
                ...
                case AddableObjects.NormalBullet:
                    ...
                    var bullet = new NormalBullet (player.Position.Sub(new Coordinate(0,10)).Add (player.Size.Div (2)).Add (new Coordinate ((float) Math.Cos (Physomatik.ToRadian (player.Rotation)) * (player.Size.Pyth / 2 + 20), (float) Math.Sin (Physomatik.ToRadian (player.Rotation)) * (player.Size.Pyth / 2 + 20))), new Coordinate (10, 30), player.Rotation,
                        engine.CurrentId, 1);
                    Objects.Add (bullet);
